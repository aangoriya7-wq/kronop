...
export class AuthService {
  ...
  async sendOTP(email: string, options: SendOTPOptions = {}) {
    try {
      ...
      if (errorMessage.includes('timeout')) {
        return { error: 'Send OTP timeout, please retry', errorType: 'timeout' };
      }
  ...
  async refreshSession() {
    try {
      ...
      console.warn('[Template:AuthService] Refresh session error:', errorMessage);
    }
  }
      ...
      if (result.type === 'success') {
        ...
        try {
          ...
          if (!code) {
            ...
            return { 
              error: `Failed to parse callback: ${errorDescription || error || 'No authorization code received'}` 
            };
          }
        ...
        return { error: 'Browser is dismissed' };
